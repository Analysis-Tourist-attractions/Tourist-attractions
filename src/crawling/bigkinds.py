...
def login():
    ...
    login=driver.find_element(
        By.CSS_SELECTOR,"#header > div.hd-top > div > div.login-area > button.btn-login.login-modal-btn.login-area-before"
    )
    login.click()

    #id 입력
    bigkinds_login=driver.find_element(
        By.CSS_SELECTOR,'#login-user-id'
        )

    bigkinds_login.send_keys(f"{login_id}")

    #pw 입력
    bigkinds_pw=driver.find_element(
        By.CSS_SELECTOR,'#login-user-password'
        )
...
def search():                             
    ...
    driver.find_element(
        By.CSS_SELECTOR,
        "#news-search-form > div > div.hd-srch.v2 > button").click()
    time.sleep(0.5)
    # 기간 버튼
    date_btn=driver.find_element(
        By.CSS_SELECTOR,"#news-search-form > div > div.hd-srch.v2 > div.srch-detail.v2 > div > div.tab-btn-wp1 > div.tab-btn-inner.tab1 > a")

    date_btn.click()
    time.sleep(0.5)
    # 시작 날짜 입력
    begin_date=driver.find_element(
        By.CSS_SELECTOR,"#search-begin-date"
    )
    begin_date.click()
    ...
    end_date=driver.find_element(
        By.CSS_SELECTOR,"#search-end-date"
    )
    end_date.click()
    ...
    search_article=driver.find_element(
        By.CSS_SELECTOR,"#search-foot-div > div.foot-btn > button.btn.btn-search.news-search-btn.news-report-search-btn"
...
def download():
    # 검색결과 탭 닫기
    driver.find_element(
        By.CSS_SELECTOR,"#collapse-step-2"
    ).click()
    time.sleep(1)
    driver.find_element(
        By.CSS_SELECTOR,"#collapse-step-3"
    ).click()
    time.sleep(2)
    # 다운로드
    driver.find_element(
