...
import pandas as pd
# 5. 예외처리 에러 임포트
from selenium.common.exceptions import ElementClickInterceptedException, NoSuchElementException, ElementNotInteractableException, NoSuchWindowException

def set_region():
    global region_list
    region_list=['광주 광산','대구 달성','성북구','성동구','인천 서구','울산 중구','동대문구','부천시','부산진구','관악구','김해시']
...
for region in region_list:
    ...
    search_bar=driver.find_element(
        By.CSS_SELECTOR,"#search\.keyword\.query"
    )
    time.sleep(0.2)
    ...
    # 엔터
    driver.find_element(
        By.CSS_SELECTOR,"#search\.keyword\.submit"
...
# -- !! 데이터를 뽑고 싶은 페이지 = range(n) 
    ...
    for change_page in range(2):                   
      ...
      for i in range(len(review_num_list)):
          ...
          try:
            # 접속 ~
            driver.find_element(By.CSS_SELECTOR,f"#info\.search\.place\.list > li:nth-child({i+1}) > div.rating.clickArea > span.score > a").send_keys(Keys.ENTER)
          ...
          # 장소 유형 추출
          try:
            loc_type=driver.find_element(By.CSS_SELECTOR,"span.txt_location").text
          ...
          if (review_num-3) %5 == 0:
            if review_num > 3:
              ...
              for click_num in range((review_num-3)//5):          
                More_Text_WebElement=driver.find_element(By.CSS_SELECTOR,"#mArticle > div.cont_evaluation > div.evaluation_review > a > span.txt_more")
                time.sleep(0.5)
            ...
            # 리뷰수가 0일때
                ...
                try :
                  check=driver.find_element(By.CSS_SELECTOR,"#mArticle > div.cont_review > div > h3")                  
                  print(f"{change_page*len(review_num_list)+i+1}번째 데이터는 리뷰수가 0건입니다.")
                  driver.close()
                except NoSuchElementException:
          ...
          print(f"{change_page*len(review_num_list)+i+1}번째 장소({place_data[change_page*len(review_num_list)+i]})리뷰 저장완료")
          time.sleep(1)

          # 데이터 정제 4. 저장
          region_data[f"{place_data[change_page*len(review_num_list)+i]}[{loc_type}]"]=review_data
      ...
      # 다음페이지 넘어가기
      try:
        driver.find_element(By.CSS_SELECTOR,f"#info\.search\.page\.no{change_page+2}").click()
